...
public class HtmlFields extends BaseModel implements ComponentExporter {
    private static Logger LOG = LoggerFactory.getLogger(HtmlModel.class);
    @SlingObject
    Resource resource;

    @JsonProperty("Content")
    private ArrayList<HtmlModel> arrHtmlModel;

    @PostConstruct
    protected void invokepost() {
        super.resource = resource.getParent();
        super.invokepost();
        arrHtmlModel = new ArrayList<HtmlModel>();
        ...
        while (iteratorExp.hasNext()) {
            ...
            HtmlModel map = childResource.adaptTo(HtmlModel.class);
            if (map != null) {
                arrHtmlModel.add(map);
            }
        }
    }
