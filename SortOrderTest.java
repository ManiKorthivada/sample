...
public class SortOrderTest {
    ...
    public void test() throws Exception{
        ...
        Mockito.when(childResource.adaptTo(ModifiableValueMap.class)).thenReturn(null);
        spySortOrder.invokepost();
    }

    @Test
    public void test_cardImage_empty() throws Exception{
        Resource parentResource = Mockito.mock(Resource.class);
        Mockito.when(resource.getParent()).thenReturn(parentResource);
        Iterator<Resource> iterator = Mockito.mock(Iterator.class);
        Mockito.when(parentResource.listChildren()).thenReturn(iterator);
        Mockito.when(iterator.hasNext()).thenReturn(false);
        MemberModifier.field(SortOrder.class,"cardImage").set(spySortOrder,"");

        spySortOrder.invokepost();
        Assert.assertEquals("",spySortOrder.getCardImage());
    }
    ...
    public void test_map_null() throws Exception{
        ...
        Assert.assertEquals("imageNewName",spySortOrder.getImageName());
        Assert.assertEquals("/content/dam/imagepath.jpeg",spySortOrder.getCardImage());
        Mockito.verify(resourceResolver,Mockito.times(0)).commit();
    }

    @Test
    public void test_answers_null() throws Exception{
        Resource parentResource = Mockito.mock(Resource.class);
        Mockito.when(resource.getParent()).thenReturn(parentResource);
        Iterator<Resource> iterator = Mockito.mock(Iterator.class);
        Mockito.when(parentResource.listChildren()).thenReturn(iterator);
        ...
        Mockito.when(childResource.getChild("answers")).thenReturn(null);
        spySortOrder.invokepost();
        Assert.assertEquals("imageNewName",spySortOrder.getImageName());
        Assert.assertEquals("/content/dam/imagepath.jpeg",spySortOrder.getCardImage());
        Mockito.verify(resourceResolver,Mockito.times(1)).commit();
    ...
    @Test
    public void test_answer_map_null() throws Exception{
        Resource parentResource = Mockito.mock(Resource.class);
        Mockito.when(resource.getParent()).thenReturn(parentResource);
        Iterator<Resource> iterator = Mockito.mock(Iterator.class);
        Mockito.when(parentResource.listChildren()).thenReturn(iterator);
        ...
        Resource answerResource = Mockito.mock(Resource.class);
        Mockito.when(iterator1.next()).thenReturn(answerResource);
        Mockito.when(answerResource.adaptTo(ModifiableValueMap.class)).thenReturn(null);
